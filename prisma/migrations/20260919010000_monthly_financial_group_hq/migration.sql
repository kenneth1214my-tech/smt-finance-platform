...
-- CSV bulk importer record a group-level P&L when it isn't broken out per subsidiary.
...
-- most one HQ-level row per (year, month).
CREATE UNIQUE INDEX "MonthlyFinancial_hq_year_month_key" ON "MonthlyFinancial" ("year", "month") WHERE "subsidiaryId" IS NULL;
