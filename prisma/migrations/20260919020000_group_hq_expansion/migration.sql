...
ALTER TABLE "SystemSetting" ADD COLUMN "equity" DECIMAL(14,2) NOT NULL DEFAULT 0;
ALTER TABLE "SystemSetting" ADD COLUMN "debtRatio" DECIMAL(6,2) NOT NULL DEFAULT 0;
...
-- most one HQ-level budget row per year. ARCustomer/Payable have no such compound-unique
-- constraint at all (each row is one customer/vendor, always freely insertable), so no
-- equivalent index is needed there.
CREATE UNIQUE INDEX "Budget_hq_year_key" ON "Budget" ("year") WHERE "subsidiaryId" IS NULL;
