...
export default async function OpsPage() {
  ...
  const [subsidiaries, monthly, systemSetting] = await Promise.all([
    db.subsidiary.findMany({ orderBy: { sortOrder: "asc" } }),
    db.monthlyFinancial.findMany({ where: { year: { in: [new Date().getFullYear() - 1, new Date().getFullYear()] } }, orderBy: { month: "asc" } }),
    db.systemSetting.findUnique({ where: { id: "default" }, select: { headcount: true } }),
  ]);
  const orgHeadcount = systemSetting?.headcount ?? 0;
  ...
  function subRow(sub: (typeof subsidiaries)[number] | typeof HQ_PSEUDO_SUB, cur: typeof mCur, prev: typeof mPrev, currentHeadcount: number) {
    ...
    // "current headcount" snapshot (Subsidiary.headcount / SystemSetting.headcount, edited under
  ...
  const bySub = [
    ...
    ...(hqCur.length || hqPrev.length ? [subRow(HQ_PSEUDO_SUB, hqCur, hqPrev, orgHeadcount)] : []),
  ];
