...
export default async function OverviewPage() {
  ...
  const [bySubsidiary, subsidiaries, systemSetting, monthlyRows, cashflow, banks, arCustomers, baseCurrency, rates] = await Promise.all([
    ...
    db.systemSetting.upsert({ where: { id: "default" }, update: {}, create: { id: "default" } }),
    db.monthlyFinancial.findMany({ where: { year: YEAR } }),
    db.cashFlowMonthly.findMany({ where: { year: YEAR } }),
    db.bankAccount.findMany(),
    db.aRCustomer.findMany(),
    getBaseCurrency(),
    getExchangeRates(),
  ]);
  ...
  // Total Assets: prefer a real balance sheet (equity + debt ratio, from the Balance Sheet import
  // or direct entry) per entity — assets = equity ÷ (1 − debtRatio%), derived from
  // debtRatio% = liabilities ÷ assets and assets = equity + liabilities. Falls back to that same
  // entity's own bank + receivables total only when it has no balance sheet data at all (equity
  // and debtRatio both still at their zero default) — the previous cash+AR proxy silently
  // ignored equity/debtRatio entirely, so an entity with a real balance sheet on file never
  // actually flowed through to this KPI.
  ...
  const investmentInSubsidiariesEliminated = Number(systemSetting.investmentInSubsidiaries);
  const dueToSubsidiaries = Number(systemSetting.dueToSubsidiaries);
  const totalAssets =
    subsidiaries.reduce((sum, s) => sum + entityAssets(s.id, Number(s.equity), Number(s.debtRatio)), 0) +
    entityAssets(null, Number(systemSetting.equity), Number(systemSetting.debtRatio)) -
