...
// otherwise it must resolve to a real subsidiary.
...
export async function POST(req: Request) {
  ...
  const [subsidiaries, regions] = await Promise.all([db.subsidiary.findMany(), db.region.findMany()]);
      ...
      switch (config.id) {
        case "monthlyFinancial": {
          ...
          if (sid) {
            ...
            // No compound-unique constraint covers (year, month) WHERE subsidiaryId IS NULL at
            // the Prisma level (only a DB-side partial index), so the HQ upsert is done by hand.
            const existing = await db.monthlyFinancial.findFirst({ where: { subsidiaryId: null, year, month } });
            if (existing) await db.monthlyFinancial.update({ where: { id: existing.id }, data: parsed.data });
            else await db.monthlyFinancial.create({ data: parsed.data });
          }
          break;
        }
        ...
        case "budget": {
          ...
          if (sid) {
            ...
            // No compound-unique constraint covers year WHERE subsidiaryId IS NULL at the
            // Prisma level (only a DB-side partial index), so the HQ upsert is done by hand.
            const existing = await db.budget.findFirst({ where: { subsidiaryId: null, year } });
            if (existing) await db.budget.update({ where: { id: existing.id }, data: parsed.data });
            else await db.budget.create({ data: parsed.data });
          }
          break;
        }
        case "bankAccount": {
          ...
          if (!parsed.success) throw new Error(parsed.error.issues[0]?.message ?? "invalid_input");
          await db.bankAccount.create({ data: parsed.data });
          break;
        }
        case "cashflow": {
        ...
        case "arCustomer": {
          ...
          const existing = await db.aRCustomer.findFirst({ where: { subsidiaryId, nameZh: parsed.data.nameZh } });
          if (existing) await db.aRCustomer.update({ where: { id: existing.id }, data: parsed.data });
          else await db.aRCustomer.create({ data: parsed.data });
          break;
        }
        case "payable": {
          const subsidiaryId = resolveSubsidiaryId(raw.subsidiaryKey, subByKey);
          ...
          if (!parsed.success) throw new Error(parsed.error.issues[0]?.message ?? "invalid_input");
          const existing = await db.payable.findFirst({ where: { subsidiaryId, nameZh: parsed.data.nameZh } });
          if (existing) await db.payable.update({ where: { id: existing.id }, data: parsed.data });
          else await db.payable.create({ data: parsed.data });
          break;
        }
        case "balanceSheet": {
          const subsidiaryId = resolveSubsidiaryId(raw.subsidiaryKey, subByKey);
          ...
          if (sid) await db.subsidiary.update({ where: { id: sid }, data: { equity: totalEquity, debtRatio } });
            ...
            await db.systemSetting.upsert({
              where: { id: "default" },
              update: { equity: totalEquity, debtRatio, investmentInSubsidiaries, dueToSubsidiaries },
              create: { id: "default", equity: totalEquity, debtRatio, investmentInSubsidiaries, dueToSubsidiaries },
            });
          break;
        }
      }
  ...
  if (successCount > 0) {
    await db.importBatch.create({ data: { fileName, rowCount: successCount, importedById: user.id } });
  }
