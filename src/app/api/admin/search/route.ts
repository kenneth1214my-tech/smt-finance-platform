...
export async function GET(req: Request) {
  ...
  const [subs, ar, ap, projects, risks] = await Promise.all([
    db.subsidiary.findMany({ where: { OR: nameMatch("nameZh", "nameEn") }, take: 5 }),
    db.aRCustomer.findMany({ where: { OR: nameMatch("nameZh", "nameEn") }, take: 5 }),
    db.payable.findMany({ where: { OR: nameMatch("nameZh", "nameEn") }, take: 5 }),
    db.project.findMany({ where: { OR: nameMatch("nameZh", "nameEn") }, take: 5 }),
    db.riskAlert.findMany({ where: { OR: [{ tag: { contains: q, mode: "insensitive" } }, { entityLabel: { contains: q, mode: "insensitive" } }] }, take: 5 }),
  ]);
