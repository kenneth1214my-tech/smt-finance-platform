...
export async function POST(req: Request) {
  ...
  if (reportKey === "consolidated_income_statement") await computeConsolidatedIncomeStatement(year, locale);
  else if (reportKey === "consolidated_balance_sheet") await computeConsolidatedBalanceSheet(year, locale);
  else if (reportKey === "consolidated_cash_flow") await computeConsolidatedCashFlow(year);
  else if (reportKey === "ar_aging") await computeARAgingReport(year, locale);
  else if (reportKey === "monthly_risk") await computeMonthlyRiskReport(year, month!, locale);
  else if (reportKey === "audit_report") await computeAuditReport(year);
  else if (reportKey === "audited_statements") await computeAuditedFinancialStatements(year, locale);
  ...
  // back one.
  const existing = await db.reportDoc.findFirst({ where: { reportKey, periodYear: year, periodMonth } });
  const row = existing
    ? await db.reportDoc.update({ where: { id: existing.id }, data: { generatedAt: new Date(), status: "GENERATED" } })
    : await db.reportDoc.create({
        data: { ...names[reportKey], type, status: "GENERATED", period, generatedAt: new Date(), reportKey, periodYear: year, periodMonth },
      });
