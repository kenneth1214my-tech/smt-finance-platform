...
// snapshot, no per-invoice detail), Budget (annual rate), Subsidiary/SystemSetting equity/debt
...
async function detectARExceptions(locale: Locale): Promise<DraftFinding[]> {
  const isZh = locale === "zh" || locale === "zh-Hant";
  const customers = await db.aRCustomer.findMany({ include: { subsidiary: true } });
...
async function detectAPExceptions(locale: Locale): Promise<DraftFinding[]> {
  const isZh = locale === "zh" || locale === "zh-Hant";
  const payables = await db.payable.findMany({ include: { subsidiary: true } });
  const findings: DraftFinding[] = [];
...
async function detectBudgetVariance(year: number, locale: Locale): Promise<DraftFinding[]> {
  const isZh = locale === "zh" || locale === "zh-Hant";
  const [budgets, subsidiaries, monthly] = await Promise.all([
    db.budget.findMany({ where: { year } }),
    db.subsidiary.findMany(),
...
function debtRatioFinding(entityLabel: string, equity: number, debtRatio: number, refKey: string, isZh: boolean): DraftFinding | null {
  ...
  return {
    ...
    evidence: `equity=${equity}, debtRatio=${debtRatio}`,
...
async function detectDebtRatioExceptions(locale: Locale): Promise<DraftFinding[]> {
  const isZh = locale === "zh" || locale === "zh-Hant";
  const [subsidiaries, setting] = await Promise.all([db.subsidiary.findMany(), db.systemSetting.upsert({ where: { id: "default" }, update: {}, create: { id: "default" } })]);
  const findings: DraftFinding[] = [];
  for (const s of subsidiaries) {
    const f = debtRatioFinding(localizedName(s, locale), Number(s.equity), Number(s.debtRatio), s.id, isZh);
    if (f) findings.push(f);
  }
  const hqF = debtRatioFinding(isZh ? "集团总部" : "Group HQ", Number(setting.equity), Number(setting.debtRatio), "hq", isZh);
  if (hqF) findings.push(hqF);
  return findings;
}

async function detectRiskRatingDivergence(year: number, locale: Locale): Promise<DraftFinding[]> {
  const isZh = locale === "zh" || locale === "zh-Hant";
  const [subsidiaries, curMonthly, prevMonthly] = await Promise.all([
    db.subsidiary.findMany(),
    db.monthlyFinancial.findMany({ where: { year } }),
    db.monthlyFinancial.findMany({ where: { year: year - 1 } }),
  ]);
  const findings: DraftFinding[] = [];
...
async function detectSegregationOfDuties(locale: Locale): Promise<DraftFinding[]> {
  const isZh = locale === "zh" || locale === "zh-Hant";
  const approvers = await db.user.findMany({ where: { status: "ACTIVE", role: { in: ["ADMIN", "DIRECTOR"] } } });
...
export async function runAuditScan(year: number, locale: Locale): Promise<{ created: number; skipped: number }> {
  const [ar, ap, budget, debt, riskDiv, sod] = await Promise.all([
    detectARExceptions(locale),
    detectAPExceptions(locale),
    detectBudgetVariance(year, locale),
    detectDebtRatioExceptions(locale),
    detectRiskRatingDivergence(year, locale),
    detectSegregationOfDuties(locale),
  ]);
  ...
  for (const d of drafts) {
    ...
    const count = await db.auditFinding.count({ where: { area: d.area, findingRef: { startsWith: `IA-${year}-${areaCode}-` } } });
...
// the live audit page.
export async function computeAuditReport(year: number): Promise<AuditReport> {
  const companyName = await getCompanyName();
  const yearStart = new Date(Date.UTC(year, 0, 1));
  const yearEnd = new Date(Date.UTC(year + 1, 0, 1));
  const findings = await db.auditFinding.findMany({
    where: { dateIdentified: { gte: yearStart, lt: yearEnd } },
