...
// record CSV the plain Bulk Import expects. Mirrors smt-finance-saas's aging-summary-parser.ts
// (built after a real Kingston export failed against the strict template) — mirrors the
// PnlImport/BalanceSheetImport pattern: read the real shape, don't invent one.
import { toNumber } from "@/lib/pnl-parser";

// Mirrors the RISK_TONE vocabulary already used on the AR/AP dashboard pages (ar/page.tsx,
// ap/page.tsx) — same thresholds as smt-finance-saas's xero-aging-parser.ts, inlined here since
// hongrui has no Xero REST API integration to import statusFromAgingDays from.
