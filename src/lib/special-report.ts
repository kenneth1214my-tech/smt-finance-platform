...
export async function computeARAgingReport(year: number, locale: Locale): Promise<ARAgingReport> {
  const [companyName, customers, monthly] = await Promise.all([
    getCompanyName(),
    db.aRCustomer.findMany({ include: { subsidiary: true }, orderBy: { balance: "desc" } }),
...
export async function computeMonthlyRiskReport(year: number, month: number, locale: Locale): Promise<MonthlyRiskReport> {
  const [companyName, subsidiaries, alerts, curYtd, prevYtd] = await Promise.all([
    getCompanyName(),
    db.subsidiary.findMany({ orderBy: { sortOrder: "asc" } }),
    db.riskAlert.findMany({ orderBy: { occurredAt: "desc" } }),
    db.monthlyFinancial.findMany({ where: { year, month: { lte: month } } }),
    db.monthlyFinancial.findMany({ where: { year: year - 1, month: { lte: month } } }),
  ]);
