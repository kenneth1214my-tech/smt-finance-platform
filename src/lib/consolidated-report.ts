...
async function getHqBalanceSheet() {
  ...
  return { equity: Number(row.equity), debtRatio: Number(row.debtRatio) };
}
...
export async function computeConsolidatedBalanceSheet(year: number, locale: Locale): Promise<ConsolidatedBalanceSheet> {
  ...
  return { companyName, year, totalAssets, totalLiabilities, totalEquity, debtRatioPct, byEntity, generatedAt: new Date() };
}
