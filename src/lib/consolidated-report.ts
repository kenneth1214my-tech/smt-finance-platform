...
async function getHqBalanceSheet() {
  const row = await db.systemSetting.upsert({ where: { id: "default" }, update: {}, create: { id: "default" } });
  return {
    equity: Number(row.equity),
    debtRatio: Number(row.debtRatio),
    investmentInSubsidiaries: Number(row.investmentInSubsidiaries),
    dueToSubsidiaries: Number(row.dueToSubsidiaries),
  };
}
...
export async function computeConsolidatedIncomeStatement(year: number, locale: Locale): Promise<ConsolidatedIncomeStatement> {
  const [companyName, subsidiaries, monthly] = await Promise.all([
    getCompanyName(),
    db.subsidiary.findMany({ orderBy: { sortOrder: "asc" } }),
    db.monthlyFinancial.findMany({ where: { year } }),
  ]);
...
// on file right now (from the latest Balance Sheet import), regardless of which year it's filed under.
...
export async function computeConsolidatedBalanceSheet(year: number, locale: Locale): Promise<ConsolidatedBalanceSheet> {
  const [companyName, subsidiaries, hq] = await Promise.all([getCompanyName(), db.subsidiary.findMany({ orderBy: { sortOrder: "asc" } }), getHqBalanceSheet()]);
  ...
  if (hq.equity !== 0 || hq.debtRatio !== 0) {
    const { totalAssets, totalLiabilities } = deriveAssetsAndLiabilities(hq.equity, hq.debtRatio);
    byEntity.push({ id: "__hq__", name: locale === "en" ? "Group HQ" : "集团总部", equity: hq.equity, debtRatio: hq.debtRatio, totalAssets, totalLiabilities });
  }
  ...
  const investmentInSubsidiariesEliminated = hq.investmentInSubsidiaries;
  const dueToSubsidiaries = hq.dueToSubsidiaries;
...
// CashFlowMonthly has no subsidiaryId (and, in this single-tenant app, no organizationId either)
// — it's tracked at the group level only, so unlike the Income Statement / Balance Sheet there's
// no per-entity breakdown available here.
export async function computeConsolidatedCashFlow(year: number): Promise<ConsolidatedCashFlow> {
  const [companyName, cashflow] = await Promise.all([getCompanyName(), db.cashFlowMonthly.findMany({ where: { year } })]);
  ...
  return { companyName, year, ocf, icf, fcf, netChange: ocf + icf + fcf, generatedAt: new Date() };
}
...
// can produce; the report page renders a clearly-labelled placeholder for it instead.
...
export async function computeAuditedFinancialStatements(year: number, locale: Locale): Promise<AuditedFinancialStatements> {
  const priorYear = year - 1;
  const [incomeStatement, priorIncomeStatement, balanceSheet, cashFlow, priorCashFlow] = await Promise.all([
    computeConsolidatedIncomeStatement(year, locale),
    computeConsolidatedIncomeStatement(priorYear, locale),
    computeConsolidatedBalanceSheet(year, locale),
    computeConsolidatedCashFlow(year),
    computeConsolidatedCashFlow(priorYear),
  ]);
